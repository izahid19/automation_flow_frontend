...
const Sidebar = ({ collapsed, setCollapsed }) => {
  ...
  const SidebarContent = ({ isMobile = false }) => {
    ...
    const itemCollapsed = !isMobile && collapsed;

    return (
      <div className="flex flex-col h-full">
        {/* Logo - Desktop Only */}
        {!isMobile && (
          <div className={`p-4 flex items-center ${itemCollapsed ? 'justify-center' : 'justify-between'}`}>
            {!itemCollapsed && (
            ...
            <Button 
              ...
              className={itemCollapsed ? '' : 'ml-auto'}
            >
              {itemCollapsed ? <PanelLeftOpen size={20} /> : <PanelLeftClose size={20} />}
            </Button>
          </div>
        )}

        {/* mobile header spacer or close button area if needed */}
        {isMobile && (
          <div className="p-6">
            <h2 className="text-lg font-bold text-foreground opacity-50 uppercase tracking-widest text-xs">
              Menu
            </h2>
          </div>
        )}
        ...
        {/* User Section */}
        <div className="p-4">
          <div className={`flex items-center gap-3 px-4 py-3 mb-2 ${itemCollapsed ? 'justify-center' : ''}`}>
          ...
          <Button
            ...
            className={`w-full justify-start text-muted-foreground hover:text-destructive hover:bg-destructive/10 ${itemCollapsed ? 'justify-center px-2' : ''}`}
            title={itemCollapsed ? "Logout" : ""}
          >
            <LogOut size={20} className={itemCollapsed ? "" : "mr-3"} />
            {showLabels && "Logout"}
          </Button>
        </div>
      </div>
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-sidebar border-b border-sidebar-border z-40 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold bg-linear-to-r from-primary to-orange-400 bg-clip-text text-transparent">
            Quote Manager
          </h1>
        </div>
        
        <Avatar className="h-8 w-8 ring-2 ring-primary/20">
          <AvatarFallback className="bg-linear-to-br from-primary to-orange-400 text-white text-xs font-bold">
            {user?.name?.charAt(0).toUpperCase()}
          </AvatarFallback>
        </Avatar>
      </div>
